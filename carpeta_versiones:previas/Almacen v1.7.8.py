...
def descargar():
    try:
        nombre_archivo = input("¿Qué nombre le quieres poner a estos registros?: ")
        ...
        print(f"Registros guardados exitosamente como '{nombre_archivo}.pkl'")
...
def cargar(nombre_archivo="registros_p.pkl"):
    ...
    try:
        ...
        print(f"Registros cargados exitosamente desde '{nombre_archivo}'")
        
    except FileNotFoundError:
        print(f"Error: No se encontró el archivo '{nombre_archivo}'")
        print("Verifica que el nombre del archivo sea correcto y que exista")
...
def crear_producto(nombre: str, precio: float, peso: float, categoria: str, stock: int) -> "Producto":
    ...
    if nombre in catalogo_productos:
        raise ValueError(f"El producto '{nombre}' ya existe en el catálogo")
    # Validar categoría
    if categoria not in guia_de_ids:
        categoria = "X"
    # Crear producto
    mercancia = Producto(nombre, precio, peso, categoria, stock)
    catalogo_productos[nombre] = mercancia
    lista_cambios.append(f"Se agregó el producto {nombre} al catálogo de mercancías con un precio de ${precio}")
    return mercancia

def crear_paquete(cantidad:int, contenido:"Producto"):
    global catalogo_prductos
    #verificar si hay stock suficiente
    if catalogo_productos[contenido._nombre].stock >= cantidad:
        contenido.quitar_producto(cantidad)
        #Crear el paquete
        ...
        raise ValueError(f"Stock insuficiente.\n Disponible: {catalogo_productos[contenido._nombre].stock} \n Solicitado: {cantidad}")

def generador_de_productos(n=20):
    if n <= 0:
        n = 1
    
    nombres_usados = set()  # Para evitar duplicados
    productos_creados = 0
    intentos = 0
    max_intentos = n * 10  # Límite para evitar bucle infinito
    
    while productos_creados < n and intentos < max_intentos:
        nombre = random.choice(NOMBRES_P)
        intentos += 1
        
        # Evitar nombres duplicados
        if nombre in nombres_usados or nombre in catalogo_productos:
            continue
            
        precio = random.uniform(1000, 99999)
        peso = random.uniform(1, 40)
        categoria = random.choice(list(guia_de_ids.keys()))
        stock = random.randint(1, 999)
        # 
        try: 
            crear_producto(nombre, precio, peso, categoria, stock)
            nombres_usados.add(nombre)
            productos_creados += 1
        except ValueError:
            # Si crear_producto falla, continúa con el siguiente
            continue
    
    print(f"Se generaron {productos_creados} productos")
